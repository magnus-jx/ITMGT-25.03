...
with open("script.json", "r") as file:
    jsondata = json.load(open("script.json"))
...
### Convert the values to a string and all characters to uppercase characters

script = {key: str(value).upper() for key, value in script.items()}

#### Remove the following pieces of text: <P>, </P>, <B>, </B>, <I>, </I>

for key, value in script.items():
    script[key] = script[key].replace("<P>","")
    script[key] = script[key].replace("</P>","") 
    script[key] = script[key].replace("<B>","") 
    script[key] = script[key].replace("</B>","") 
    script[key] = script[key].replace("<I>","")
    script[key] = script[key].replace("</I>","")
    script[key] = script[key].replace("\n","")

#### Convert all the unnecessary punctuation symbols denoted in the string below to a " "

unnecessary_punctuation = r"&$@[].,'#()-\"!?’_;:/"

for key, value in script.items():
    script[key] = script[key].translate({ord(i): " " for i in "&$@[].,'#()-\"!?’_;:/"})
...
for i in script
    for j in script.values()
        if script[i[j]] == " "
            if len(word_so_far) > 3:
...
# I am trying to find first the individual words from all the key-value pairs and creating a list that I can manipulate for the succeeding steps.
...
constraint1 = lambda f, r, c: 1000 * f + 600 * r + 750 * c <= 18000
constraint2 = lambda f, r, c: 500 * f + 750 * r + 75 * c <= 12000
constraint3 = lambda f, r, c: 50 * f + 45 * r + 75 * c <= 1500
objective_function = lambda f, r, c: 45 * f + 30 * r + 25 * c

for f in range(30 + 1):
    for r in range(33 + 1):
        for c in range(40 + 1):  # All these ranges were obtained by setting the other variables as 0.

            if constraint1(f, r, c) and constraint2(f, r, c) and constraint3(f, r, c):
                current_Z = objective_function(f, r, c)

                if current_Z >= optimal_Z:
                    optimal_Z = current_Z
                    optimal_F = f
                    optimal_R = r
                    optimal_C = c
...
print("Optimal Fighters (F):", 50 * optimal_F)
print("Optimal Rangers (R):", 45 * optimal_R)
print("Optimal Clerics (C):", 25 * optimal_C)
...
def Collatz(start_number):
    ...
    collatz_string = [start_number]
    
    while start_number > 1:

        if start_number % 2 == 0:
            start_number //= 2
            collatz_string.append(start_number)
        
        else:
            start_number = (start_number * 3) + 1
            collatz_string.append(start_number)

    result['collatz_sequence'] = collatz_string
    result['collatz_length'] = len(collatz_string)
    result['max_collatz'] = max(collatz_string)
    result['sequence_sum'] = sum(collatz_string)
...
def Collatz_winner(number_list):
    ...
    for candidate in number_list:
        ...
        collatz_string = [candidate]
        
        while candidate > 1:
            if candidate % 2 == 0:
                candidate //= 2
                collatz_string.append(candidate)

            else:
                candidate = (candidate * 3) + 1
                collatz_string.append(candidate)
    
        candidate_result['collatz_length'] = len(collatz_string)
        candidate_result['max_collatz'] = max(collatz_string)
        
        result[collatz_string[0]] = candidate_result
    
    max_length = max(result[num]['collatz_length'] for num in result)
    max_value = max(result[num]['max_collatz'] for num in result)
    
    winners = [num for num in result if result[num]['collatz_length'] == max_length and result[num]['max_collatz'] == max_value]
